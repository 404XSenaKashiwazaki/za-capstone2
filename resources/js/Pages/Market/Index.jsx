...
const IndexPage = ({title, d, users}) => {
    ...
    useEffect(() => {
        const getProdi = async () => {
            try {
                const { data } = await axios.post(route("market.search"),{ search })
                setDatas(data)
            } catch (error) {
                console.log(error);
        ...
        return (data && data.length > 0) ? data.map((d) => (
            <tr key={d.id} className="p-0 text-start h-20">
                <td className="px-6 py-1 whitespace-nowrap text-sm font-medium text-gray-900">
                    <div className="grid grid-cols-1 sm:grid-cols-[50px_1fr] items-center">
                ...
                { users.role == "admin" && <td className="px-6 py-2 whitespace-nowrap text-sm text-gray-500 ">
    ...
    <div className="py-12">
       ...
       { users.role == "admin" &&  <div className="flex justify-end w-full gap-2 items-center">
            ...
            <div className="overflow-hidden bg-white shadow-sm sm:rounded-lg dark:bg-gray-800">
                ...
                <TableComponent td={tableBody(datas)} th={["Produk","Permintaan pasar","harga",users.role == "admin" && "Action"]} />
            </div>
        </div>
        ...
        <Modal onClose={() => setIsOpenDelete(false)} show={isOpenDelete} setDataForm={setData} setId={setId}>
            <DeletePage type={type} data={data} nama={nama} setId={setId} setNama={setNama} setIsOpen={setIsOpenDelete} id={id} />
        </Modal>
    </div>
</AuthenticatedLayout>
}
